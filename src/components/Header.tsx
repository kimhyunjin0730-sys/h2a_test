...
const MENUS: { id: string; label: React.ReactNode; links: MenuLink[] }[] = [
  ...
  { id: 'mega-programs', label: '프로그램', links: [{ heading: '개인 프로그램' }, { href: '/appearance', label: 'HWANG JUNGSUN Academy' }, { href: '/soul-speech', label: 'Soul Speech ACADEMY' }, { heading: '기업교육' }, { href: '/business', label: '기업교육 소개' }, { href: '/business/solution', label: <>H<sup>2</sup>A</> }, { href: '/business/quantum', label: 'Quantum 품격' }, { href: '/business/proposal', label: '기업교육 제안 요청' }] },
        ...
        {MENUS.map((m) => (
          <div className="m-nav-item" key={m.id}>
            ...
            <div className={`m-nav-body acc-content${openAcc === m.id ? ' active' : ''}`}>
              {m.links.map((l, i) => l.heading ? <span key={'h' + i} className="m-heading">{l.heading}</span> : <Link key={l.href} href={l.href as string} className="m-link" onClick={() => setMobileOpen(false)}>{l.label}</Link>)}
            </div>
          </div>
        ))}
